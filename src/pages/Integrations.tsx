...
export default function Integrations(){
 ...
 return <div>
  <div className="page-head"><div><h1>Conexões</h1><p>Configure os serviços externos utilizados pelo escritório e pelos módulos do LEXOFFICE.</p></div><span className="integration-pill"><Plug size={15}/> CONEXÕES DO ESCRITÓRIO</span></div>
  <section className="integration-panel">
   <h3><MessageCircle size={17}/> WhatsApp</h3>
   <div className="integration-head"><div><span className="eyebrow">CONEXÃO DO ESCRITÓRIO</span><h2>{waConnected?'WhatsApp conectado':'Conectar WhatsApp ao LEXOFFICE'}</h2><p>{waConnected?'Canal disponível para atendimento, automações e agentes de IA.':'A conexão é uma configuração administrativa do escritório e não interfere na tela diária de conversas.'}</p></div><span className={waConnected?'conn-status on':'conn-status'}>{waConnected?<CheckCircle2 size={13}/>:<RefreshCw size={13}/>} {waConnected?'Conectado':waState}</span></div>
   ...
   {error&&<div className="status-message error"><AlertCircle size={16}/>{error}</div>}{notice&&<div className="status-message success"><CheckCircle2 size={16}/>{notice}</div>}
  </section>
  ...
  <div className="module-box"><h3>Arquitetura integrada</h3><p>As conexões são reutilizadas pelos módulos e pelos agentes de IA. Credenciais sensíveis não são exibidas nesta central.</p></div>
 </div>
}
