...
export default function Consultations(){
 ...
 async function run(tool:Tool){setSelected(tool);setNotice('');setInternal([]);const q=query.trim();if(!q){setNotice('Digite o dado que deseja investigar.');return}if(tool.key==='namecpf'){
   if(!supabase){setNotice('Banco indisponível.');return}setBusy(true);try{const term=q.replace(/\D/g,'');let req=supabase.from('clients').select('id,name,cpf_cnpj,rg,birth_date,profession,address,phone,whatsapp,email,status').limit(50);req=term.length>=6?req.ilike('cpf_cnpj',`%${term}%`):req.ilike('name',`%${q}%`);const {data,error}=await req;if(error)throw error;setInternal(data||[]);setNotice((data||[]).length?`${data!.length} registro(s) encontrado(s) no LEXOFFICE.`:'Nenhum registro encontrado no cadastro interno.');}catch(e:any){setNotice(e?.message||'Falha na pesquisa interna.')}finally{setBusy(false)}return}
 const url=tool.run(q);if(url)window.open(url,'_blank','noopener,noreferrer');
 }
 return <div className="module"><div className="page-head"><div><h1>Investigação</h1><p>Central de investigação de dados e OSINT para pessoas, empresas, veículos, internet, imagens e documentos.</p></div></div><div className="system-bar"><span><ShieldCheck size={14}/> INVESTIGAÇÃO DE DADOS</span><span className="online">● FONTES PÚBLICAS E DADOS AUTORIZADOS</span></div>
 <div className="integration-panel"><h3><Search size={17}/> Dado a investigar</h3><div className="integration-form"><label className="wide">Nome, CPF, CNPJ, telefone, e-mail, usuário, placa, domínio, IP ou termo<input value={query} onChange={e=>setQuery(e.target.value)} placeholder={selected?.placeholder||'Digite o dado que deseja investigar'}/></label></div><div className="campaign-tabs" style={{marginTop:12}}>{cats.map(c=><button key={c} className={category===c?'active':''} onClick={()=>setCategory(c)}>{c}</button>)}</div></div>
 <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(280px,1fr))',gap:14}}>{visible.map(tool=>{const I=tool.icon;return <button key={tool.key} className={`integration-panel ${selected?.key===tool.key?'selected':''}`} style={{textAlign:'left',cursor:'pointer',minHeight:175}} onClick={()=>run(tool)}><div style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:12}}><div className="card-icon"><I size={20}/></div><ExternalLink size={15}/></div><h3 style={{marginTop:14}}>{tool.title}</h3><p>{tool.description}</p>{tool.note&&<small style={{display:'block',marginTop:8,color:'#9ca5b4'}}>{tool.note}</small>}</button>})}</div>
 {notice&&<div className="integration-notice" style={{marginTop:16}}>{busy?'Pesquisando...':notice}</div>}
 {internal.length>0&&<div className="integration-panel" style={{marginTop:16}}><h3><Database size={17}/> Dados encontrados no LEXOFFICE</h3><div className="signature-list">{internal.map(x=><div className="signature-row" key={x.id}><div><strong>{x.name}</strong><small>CPF/CNPJ: {x.cpf_cnpj||'—'} · RG: {x.rg||'—'} · Telefone: {x.phone||x.whatsapp||'—'}</small><small>E-mail: {x.email||'—'} · Profissão: {x.profession||'—'}</small><small>Endereço: {x.address||'—'}</small></div></div>)}</div></div>}
 <div className="integration-panel" style={{marginTop:16}}><h3><ShieldCheck size={17}/> Uso responsável</h3><p>Esta área centraliza pesquisas em fontes públicas e dados do próprio escritório. Consultas que dependam de bases restritas, credenciais governamentais ou fornecedores pagos só devem ser habilitadas por integração oficial/autorizada.</p></div></div>
}
