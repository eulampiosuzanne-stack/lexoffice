...
export default function ProcessMovements(){
 const [rows,setRows]=useState<any[]>([]),[queue,setQueue]=useState<any[]>([]),[q,setQ]=useState(''),[loading,setLoading]=useState(true),[updated,setUpdated]=useState<Date|null>(null),[acting,setActing]=useState<string|null>(null),[notice,setNotice]=useState('');
 async function load(){
  if(!supabase)return;
  setLoading(true);
  setNotice('');
  ...
  const [m,n]=await Promise.all([
   supabase.from('process_movements')
    .select('id,org_id,process_id,movement_date,title,description,source,client_message,approved_for_client,sent_to_client_at,created_at,processes(cnj_number,internal_number,subject,clients(name))')
    .eq('org_id',orgId)
    .order('created_at',{ascending:false})
    .limit(1000),
   supabase.from('process_notification_queue')
    .select('id,process_id,client_id,movement_id,notification_type,channel,scheduled_at,message_text,status,last_error,sent_at,clients(name),processes(cnj_number,internal_number)')
    .eq('org_id',orgId)
    .order('scheduled_at',{ascending:false})
    .limit(1000)
  ]);
  if(m.error)setNotice(`Erro ao carregar andamentos: ${m.error.message}`);
  else setRows(m.data||[]);
  if(n.error)setNotice(v=>v?`${v} · Fila: ${n.error.message}`:`Erro ao carregar fila: ${n.error.message}`);
  else setQueue(n.data||[]);
  setUpdated(new Date());
  setLoading(false);
 }
 ...
 const reviewQueue=queue.filter(x=>x.status==='awaiting_review'),pendingQueue=queue.filter(x=>['pending','processing'].includes(x.status)),sentQueue=queue.filter(x=>x.status==='sent');
 return <div className="module movement-premium">
  <div className="page-head movement-page-head"><div><span className="movement-kicker">SF · INTELIGÊNCIA PROCESSUAL</span><h1>Movimentações Processuais</h1><p>Veja claramente o que já foi enviado automaticamente e o que depende da sua aprovação.</p></div><button className="primary movement-refresh" onClick={load}><RefreshCw size={20}/> ATUALIZAR</button></div>
