...
export default function Finance(){
 ...
 async function add(){
  ...
  setNotice('');
  const total=parseBRL(form.amount);
  ...
  try{
   if(form.launch_kind==='fee_contract'){
    ...
    if(form.has_down_payment){const entry=parseBRL(form.down_payment_amount);if(!entry||entry<=0||entry>=total)throw new Error('A entrada deve ser maior que zero e menor que o valor total.');}
    ...
    const{data,error}=await supabase.rpc('create_fee_contract_with_financial_entries_v2',{
     p_client_id:form.client_id,p_title:form.description.trim(),p_service_description:form.service_description.trim()||null,p_total_amount:total,p_payment_model:paymentModel,
     p_down_payment_amount:form.has_down_payment?parseBRL(form.down_payment_amount):0,p_down_payment_date:form.has_down_payment?(form.down_payment_date||null):null,
     p_installments_count:form.payment_mode==='single'?1:Math.max(1,Number(form.installments)||1),p_first_due_date:form.first_due_date||null,p_due_day:form.due_day?Number(form.due_day):null,p_process_id:form.process_id||null,p_notes:null
    });
    if(error)throw error;const count=Number((data as any)?.financial_entries_created||1);setNotice(`Contrato criado e integrado ao Financeiro. ${count} cobrança(s) lançada(s).`);
   }else{
    ...
    const{error}=await supabase.from('financial_entries').insert(payload);if(error)throw error;setNotice('Lançamento financeiro criado com sucesso.');
   }
 ...
 return <>
  ...
  <div className="integration-panel"><div className="integration-form">
   ...
   <label>Valor total<input type="text" inputMode="decimal" value={form.amount} onChange={e=>setForm({...form,amount:e.target.value})} placeholder="Ex.: 20.000,00"/></label>
   {form.launch_kind==='fee_contract'?<><label>Forma<select value={form.payment_mode} onChange={e=>setForm({...form,payment_mode:e.target.value,has_down_payment:false})}><option value="single">À vista</option><option value="installments">Parcelado</option></select></label>
