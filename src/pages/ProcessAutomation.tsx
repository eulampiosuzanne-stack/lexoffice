...
export default function ProcessAutomation(){
 ...
 async function loadMovements(currentOrgId:string){
  ...
  const {data,error}=await supabase.from('process_movements')
   ...
   .gte('movement_date',cutoff.toISOString())
 ...
 useEffect(()=>{
  ...
  const channel=supabase.channel(`process-movements-${orgId}`)
   .on('postgres_changes',{event:'INSERT',schema:'public',table:'process_movements',filter:`org_id=eq.${orgId}`},(payload:any)=>{const d=payload?.new?.movement_date?new Date(payload.new.movement_date):null;const cutoff=new Date();cutoff.setDate(cutoff.getDate()-30);if(d&&d>=cutoff){setLive(true);loadMovements(orgId)}})
 ...
 return <div>
  ...
  <div className="integration-panel"><div className="integration-head"><div><span className="integration-pill"><Activity size={14}/> ATUALIZAÇÕES</span><h2>Andamentos processuais recentes</h2><p>Exibe a data real do andamento. Importações históricas da OAB/DataJud não são tratadas como novidade.</p></div></div>{movements.length===0?<div className="integration-empty"><Activity size={28}/><p>Nenhum andamento dos últimos 30 dias.</p></div>:<div className="signature-list">{movements.map(x=><a className="signature-row" href={`/processos?processo=${x.process_id}`} key={x.id}><div><strong>{x.title||'Movimentação processual'}</strong><small>{proc(x)} • {cli(x)} • andamento: {dt(x.movement_date)} • importado: {dt(x.created_at)} • {x.source||'fonte não informada'}</small>{x.description&&<p style={{margin:'6px 0 0'}}>{String(x.description).slice(0,300)}</p>}</div><Gavel size={16}/></a>)}</div>}</div>
