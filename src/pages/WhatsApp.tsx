...
function findImage(value:any):string|null{
  ...
  if(typeof value==='object'){
    for(const k of ['value','image','base64','qrCode','qrcode','qr_code','Qrcode','code']){
...
export default function WhatsApp(){
  ...
  async function invokeEvolution(body:any){
    if(!supabase)return {data:null,error:new Error('Supabase indisponível.')};
    // O navegador envia somente a ação/dados funcionais.
    // Nenhum token, API key ou credencial da Evolution sai do frontend.
    return supabase.functions.invoke('whatsapp-evolution-connect',{body});
  }

  async function checkStatus(silent=false){
    if(!supabase)return false;
    const r=await invokeEvolution({action:'status'});
    if(r.error||!r.data?.ok){
      if(!silent)setError(await getFunctionError(r,'Não foi possível consultar a Evolution API.'));
      setState('indisponível');
      return false;
    }
    setConnected(!!r.data.connected);
    setState(r.data.state||'desconectado');
    if(r.data.connected){setQr(null);setNotice('WhatsApp conectado pela Evolution API.');}
    return !!r.data.connected;
  }

  useEffect(()=>{checkStatus(true)},[]);
  ...
  async function generateQr(){
    ...
    if(await checkStatus(true)){setLoading(false);return}
    const r=await invokeEvolution({action:'qr'});
    setLoading(false);
    if(r.error||!r.data?.ok){setError(await getFunctionError(r,'Não foi possível gerar o QR Code pela Evolution API.'));return}
    if(r.data.connected){setConnected(true);setQr(null);setNotice('WhatsApp já está conectado pela Evolution API.');return}
    const image=findImage(r.data.qr_code??r.data);
    if(!image){setError('A Evolution respondeu, mas não retornou uma imagem de QR Code válida.');return}
    setQr(image);setState(r.data.state||'qr');setNotice('QR Code Evolution gerado. Escaneie em WhatsApp > Aparelhos conectados.');
  }
