...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    const keepAiActive=b.keep_ai_active!==false;
    ...
    for(const t of unique){try{
      ...
      if(t.conversation_id){
        ...
        }else if(!ctx.internal){patch.last_human_outbound_at=now}
        await a.from('whatsapp_conversations').update(patch).eq('id',t.conversation_id).eq('org_id',ctx.orgId);
      }
