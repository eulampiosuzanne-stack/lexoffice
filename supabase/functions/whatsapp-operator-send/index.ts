...
const HUMAN_COOLDOWN_MINUTES = 20;
function admin() {
  return createClient(
...
async function sendViaGate(
  ...
  const r = await fetch(`${u}/functions/v1/whatsapp-outbound-gate`, {
    method: "POST",
    headers: {
    ...
    body: JSON.stringify({
      ...
      phone,
      agent_key: AGENTS.has(agentKey) ? agentKey : "system",
      idempotency_key: key,
      message,
  ...
  if (!r.ok || !d?.ok || d?.allowed === false)
    throw new Error(d?.error || d?.reason || "Falha no envio pela Z-API");
  return d;
}
async function setConversationAiState(
  ...
  const resumeAt = new Date(
    now.getTime() + HUMAN_COOLDOWN_MINUTES * 60 * 1000,
  ).toISOString();
    ...
    if (key) {
      const controlPatch = keepAiActive
        ...
        : {
            ...
            resume_after_minutes: HUMAN_COOLDOWN_MINUTES,
            resume_at: resumeAt,
            updated_at: nowIso,
          };
      await a
        ...
        .upsert(
          { org_id: orgId, contact_key: key, ...controlPatch },
          { onConflict: "org_id,contact_key" },
        );
    }
  }
}
Deno.serve(async (req) => {
  ...
  try {
    ...
    const nextAgentRaw = String(
      b.next_agent_key || requestedAgent || "",
    ).trim();
    ...
    if (targets.length > 200)
      return json(
        { ok: false, error: "Limite de 200 destinatários por envio" },
        400,
      );
    const seen = new Set<string>();
    const unique = targets
      .map((x: any) => ({
        phone: digits(x.phone),
    ...
    for (const t of unique) {
      try {
        ...
        const externalId =
          String(
            d?.provider_response?.messageId || d?.provider_response?.id || "",
          ) || null;
        if (t.conversation_id) {
          ...
          await a
            ...
            .insert({
              ...
              sent_at: now,
              metadata: {
                source: "zapi_operator_send",
                provider: "zapi",
                fromMe: true,
                sender_type: keepAiActive ? "system" : "human",
                operator_user_id: ctx.userId,
                keep_ai_active: keepAiActive,
                next_agent_key: nextAgent,
                human_cooldown_minutes: keepAiActive
                  ? 0
    ...
    if (m === "FORBIDDEN")
      return json(
        { ok: false, error: "Usuário sem acesso ao escritório" },
        403,
      );
    if (m === "INTERNAL_ORG_REQUIRED")
      return json(
        { ok: false, error: "Organização obrigatória para envio interno" },
        400,
      );
